...
截至 2026-09-18，**84 项单元与集成测试、类型检查、生产构建及 4 项平台浏览器测试通过**。真实模型已生成待办、计算器和计时器；已验证同一计算器连续两次修改、无效输入保护、刷新保存、源码导出与线上代码恢复。Supabase 的真实并发、原子写入和匿名会话隔离全部通过。详细范围和未覆盖项见 [验收记录](docs/acceptance.md)。
...
两个页面基于同一份旧数据保存时，后一个请求可能返回 409，避免覆盖先保存的数据。工作台会暂停旧预览写入，并在预览区显示“载入最新数据”。点击后会从服务器读取最新项目，再重建预览；用户可重新操作。失败的旧值不会自动重放。普通“重新加载预览”也会读取最新项目，读取失败时保留重试入口。
...
Playwright 配置会启动启用开发样例的本地服务器。如果已有服务器占用测试地址，应确保它以 `ENABLE_DEV_FIXTURES=true` 启动。这 4 项测试覆盖真实浏览器中的发布、数据隔离、刷新、失败回滚与跨页面保存冲突恢复，不调用付费模型，已全部通过。
