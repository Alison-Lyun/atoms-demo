...
截至 2026-09-18，**82 项单元与集成测试、类型检查、生产构建及 4 项平台浏览器测试通过**。真实模型已生成待办、计算器和计时器；已验证同一计算器连续两次修改、无效输入保护、刷新保存、源码导出与线上代码恢复。Supabase 的真实并发、原子写入和匿名会话隔离全部通过。详细范围和未覆盖项见 [验收记录](docs/acceptance.md)。
