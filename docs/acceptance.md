...
| 检查 | 结果 | 已验证范围 |
| --- | --- | --- |
| 单元与集成测试 | 84 项通过 | 生命周期、存储、生成状态机、静态校验、HTTP、模型输出解析 |
| TypeScript / Next.js build | 通过 | 最终 TypeScript 类型检查与 Next.js 生产构建均通过 |
| 依赖审计 | 0 个已知漏洞 | 新目录 `npm ci --legacy-peer-deps`，锁文件安装 |
| 平台浏览器测试 | 4/4 通过，0 flaky | 真实 Chrome 预览发布、持久化与隔离、候选暂存、运行时失败回滚及保存冲突恢复 |
...
84 项单元与集成测试包含真实临时文件重开、会话隔离、40 路并发更新和抛错回滚。生命周期测试覆盖版本晋升、数据 revision 冲突、失败保留、取消及超时；新增两项完整路径测试分别验证静态错误和候选启动错误连续失败时，初次调用加两次修复后停止，旧代码与业务数据保持不变，迟到成功回报不能晋升。engine 使用模拟模型与存储竞态测试幂等、有限重试和取消后的迟到结果。模拟检查不代表已进行云端取消或供应商错误注入测试。

正式 Chrome 平台运行在独立报告目录完成，耗时约 3.8 分钟，4 个用例全部通过：

1. fixture 经真实预览回报发布，新增数据在刷新后保留，并与另一项目隔离。
2. 候选写入在验证提交前不可见，提交后才进入当前项目。
3. 候选启动抛出真实运行时异常后失败，暂存数据不进入当前项目。
4. 6 次并发存储调用依次保存，全部 HTTP 200；两页面制造过期数据 409 后云端数据保持完整，旧预览无法再发写请求。手机与全屏均可见恢复按钮；模拟首次读取 503 后仍能重试，载入最新数据后明确重新操作保存成功。普通刷新预览也会真正 GET 最新项目，之后继续保存成功。
...
用户界面的恢复缺口也已修复：旧“刷新预览”只重建 iframe，现在会等待在途保存结束后读取最新项目，再重建预览。409 后暂停旧写入，在预览区提供中文说明和“载入最新数据”；不自动重放过期值，读取失败仍保留恢复按钮。这条流程已通过上述双页面浏览器回归，并在真实线上计算器完成 1/1 复测：实际保存状态为 200 → 409 → 载入最新 → 200 → 200，历史由 2 条增长为 5 条且全部保留；没有自动重放旧值，也没有调用模型。证据见 [线上冲突恢复](evidence/production-conflict-checks.json)。
...
- 静态错误与候选启动错误修复耗尽、取消后的迟到结果、180 秒截止状态有确定性测试；真实模型生成均无需修复，尚未实测真实模型修复成功、真实供应商 401/429、真实请求超时或跨实例取消。已发布应用在用户交互后发生运行错误时会显示提示，不自动进入修复循环。
- 待办的真实交互验收覆盖新增、搜索、刷新和代码恢复；未把完成/删除操作列为已实测。
